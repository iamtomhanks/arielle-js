import { OpenAPISpec, OperationObject, ParameterObject, ReferenceObject, RequestBodyObject, PathItemObject } from './types.js';
 ...
 */
export function processOpenAPISpec(spec: OpenAPISpec): ProcessedEndpoint[] {
 ...
 */
function processOperation(params: ProcessOperationParams): ProcessedEndpoint | null {
...
function resolveParameters(
  ...
  components?: { parameters?: Record<string, ParameterObject | ReferenceObject> }
): ParameterObject[] {
  return parameters.map(param => {
    ...
    if ('$ref' in param) {
      ...
      if (!ref.startsWith('#/components/parameters/')) {
        ...
        return { name: 'unknown', in: 'query' } as ParameterObject;
      }
      ...
      if ('$ref' in resolvedParam) {
        ...
        return { name: paramName, in: 'query', ...resolvedParam } as ParameterObject;
      }
      
      return resolvedParam as ParameterObject;
    }
...
function processRequestBody(
  ...
  components?: { requestBodies?: Record<string, RequestBodyObject | ReferenceObject> }
): RequestBodyInfo | null {
  // Handle request body reference
  let resolvedBody: RequestBodyObject;
  ...
  if ('$ref' in requestBody) {
    ...
    resolvedBody = resolved as RequestBodyObject;
  } else {
...
function generateNlpText(params: {
  ...
  if (requestBody) {
    parts.push(
      `request body: ${requestBody.description || ''}`,
      ...(requestBody.contentTypes || []).map(ct => `content type: ${ct}`)
    );
  }
...
// Additional type definitions for processed data
