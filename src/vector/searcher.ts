 ...
 */
export async function findSimilarEndpoints(
  endpoint: { path: string; method: string },
  options?: Omit<SearchOptions, 'includeMetadata'>
): Promise<VectorSearchResult[]> {
  const collection = await getCollection();
  ...
  try {
    ...
    for (let i = 0; i < (similar.ids?.[0]?.length || 0); i++) {
      const id = similar.ids[0][i] as string;
      ...
      if (score < (options?.minScore || DEFAULT_MIN_SCORE)) continue;
      ...
      if (searchResults.length >= (options?.limit || DEFAULT_LIMIT)) break;
    }
